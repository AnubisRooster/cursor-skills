...
description: >
  ...
  Any human-readable Confluence prose on the page (intro, captions, process notes)
  must follow the writing-voice skill.
---
...
2. **Draft page prose with writing-voice** (`~/.claude/skills/writing-voice/SKILL.md`)
   before create/update. Formal Confluence voice for intros, captions, and notes.
   The HTML artifact itself is exempt; surrounding wiki text is not.
3. **Create/locate the page** (`confluence_create_page` / `confluence_update_page`,
...
4. **Attach the HTML** (`confluence_upload_attachment`). If the DC instance
...
5. **Wire the body** to the attachment filename: a direct `<ac:link>` to the
   `<ri:attachment>` plus an `html` macro iframe to
   `/download/attachments/<pageId>/<filename>`.
6. **Generate the fragment** as a paste-in fallback for the HTML macro.
